...
def talk(driver,url):
    ...
    try:
        ...
        flag=True
    while(talkcount<200):  #循环同级出200条评论
        ...
        for i in li:
            if flag==True and 'list_li' in i.get_attribute('class'):
                ...
                texta=texta.get_attribute('href')
                els=i.find_element_by_css_selector('span')
                lens=0  #转发分级
                for j in els.text:
                    if '@' in j:lens+=1
                talkurl[texta] = [lens+1, textatime]
		#print(texta,talkurl[texta])
                #print("texta:",texta)
                #print("text:",talkurl[texta])
				
                #talkurl.append(texta)
                print(userName+':'+a+' 转发'+str(lens+1)+'级转发:')
                talkcount+=1
                if talkcount==200:break
            if 'between_line' in i.get_attribute('class'):
                flag=True
        driver.find_element_by_class_name('next').click()
        sleep(5)
        #driver.implicitly_wait(10)
    return talkurl, href, textatime
